...
The recovered import loop accepts a parsed secondary-exit record only when its index is below
`$2000`; otherwise it takes Lunar Magic's error path before writing any of the six global planes.
Rust therefore rejects `$2000+` indices during both binary-MWL decode and encode. A reciprocal live
oracle covers `$0000`, `$1FFF`, every packed-field maximum, installed-table reopen, empty-set clear,
and final checksum validity.
