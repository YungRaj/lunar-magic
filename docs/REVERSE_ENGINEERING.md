...
independent Maintain Detail switch that skips it. The Wine harness captures both control states;
the retained 16-color high-color fixture is byte-identical in both modes, while a focused palette-
pressure test proves the branch changes partial-set extension without changing exact-fit behavior.
