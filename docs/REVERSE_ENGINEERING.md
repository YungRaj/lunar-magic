...
The external-tool registry contract is now recovered end to end. `LoadExternalToolProfileAFromRegistry`/`SaveExternalToolProfileAFromRegistry` select `Emulator`/`EmulatorArg` or `Emulator2`/`Emulator2Arg` according to the active SNES/GBA family, while profile B owns `TileEditor`/`TileEditorArg`; a missing argument value becomes `"%1"` and a missing executable clears that profile. `SynchronizeApplicationSettingsRegistry` binds SNES custom arguments to `Options` bit 29; SNES short paths, GBA short paths, GBA custom arguments, and tile-editor custom arguments to `Options2` bits 16, 17, 18, and 24 respectively. `LaunchGraphicsFileExternalEditor` independently proves that bit 24 selects `ExpandExternalEditorCommandTemplate`, while a clear bit passes the quoted graphics path directly. `SetupExternalTileEditorDialogProc` at `$00406670` proves controls `$67` (replace `yychr.pal`) and `$6B` (set transparent colors to blue) are mutually exclusive presentations of the same state byte: clicking either clears its peer, acceptance stores `$67`, and initialization derives `$6B` as its inverse. `SynchronizeApplicationSettingsRegistry` maps that byte to `Options2` bit 25. The launch path tests the bit before creating `yychr.pal`; a clear bit leaves the file absent so the external editor uses its blue transparency presentation. This behavior is independent of the launch template.
...
The unrelated level-editor option `LM_OPTIONS_ALLOW_FRAGMENT` is command `$24BA`, central
dispatcher case `$85`, and byte `$005E76EB`; `SynchronizeApplicationSettingsRegistry` stores it as
`Options` bit 1 and the live default is one. Its only editing xrefs are
`ReinsertMovedLevelObjectsByTileDelta`, `ReinsertMovedLevelObjectsInReverseOrder`,
`MoveSelectedObjectsTowardListHead`, and `MoveSelectedObjectsTowardListTail`. The head/tail
decompilations compare each candidate's screen-position word at node offset `$1E`: when positions
differ they call `InsertLevelObjectBetweenScreenPositions` only while `$005E76EB` is nonzero;
otherwise the operation stops at that anchor. The reinsertion paths similarly rebuild the Map16
source-object map, then repair selected painter order across screen anchors only while enabled.
Thus “fragmentation” means repeated/backtracking screen-position fragments in the level object
list, not fragmentation of ROM allocation space.
