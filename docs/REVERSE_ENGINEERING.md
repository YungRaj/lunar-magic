...
and four sprite `GraphicsFile4bpp` payloads in native slot order, with errors retaining the exact
slot and file number. Disabled bypasses perform no pointer reads. The live ROM assets editor exposes
this resolver as an explicit validation action and reports aggregate foreground and sprite tile
counts; this establishes the safe materialization boundary needed by the level rasterizer.
