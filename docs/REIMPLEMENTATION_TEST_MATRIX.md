...
preview/session path. BMP decoding is bounded to uncompressed single-plane 24-bit BGR or 32-bit
BGRX, accepts padded top-down and bottom-up rows, treats the unused BGRX byte as opaque, and rejects
bad signatures, DIB extents, dimensions, planes, depth, compression, offsets, arithmetic overflow,
or truncated pixels without producing a partial image.
