...
preview/session path. BMP decoding is bounded to uncompressed single-plane indexed 1-/4-/8-bit,
24-bit BGR, or 32-bit BGRX. It decodes packed indexed pixels through bounded BGRA palette tables,
accepts padded top-down and bottom-up rows, treats reserved palette/BGRX bytes as opaque, and rejects
bad signatures, DIB extents, dimensions, planes, depth, compression, palette shapes or indexes,
offsets, arithmetic overflow, or truncated pixels without producing a partial image.
