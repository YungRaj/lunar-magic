...
| Lunar restore points | Decode the exact `$130`-byte `LR` archive prefix and linked `$100`-byte `DIRL` records with 64-bit reciprocal links, record IDs, packed timestamps, ROM sizes, descriptions, payload bounds, raw-DEFLATE flag, decoded byte-sum checksum, and logical-ROM CRC-32; decode compact fill/raw commands with 24/32-bit destinations and one- through four-byte lengths | Reconstruct a selected record by applying the complete linked delta chain to the original ROM and resizing to every record's declared ROM size; synthetic compressed commands pass, an authenticated Lunar Magic 3.63 two-record archive expands to 525 commands and reproduces the 2,097,664-byte target ROM exactly, and the native closed-project workflow lists all recovered columns then publishes the chosen record off-thread through rollback-safe replacement | Reject short/bad headers, invalid signatures, record cycles, broken previous links, mismatched last links, overlapping/out-of-bounds headers, descriptions, and payloads, failed inflation/checksum/ROM hash, malformed/truncated/unterminated commands, trailing decoded bytes, arithmetic overflow, unknown record IDs, aliased archive/original/target paths, non-regular or missing replacement targets, and outputs above the 16-MiB ROM bound |
