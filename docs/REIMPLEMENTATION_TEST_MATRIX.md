...
An application-backed SSC catalog fixture also selects independent four-, five-, six-, and
seven-byte custom records across all four extra-bit tables, preserves current placement, verifies
every zero-filled extension, semantically edits packed coordinates without changing any width or
table selector, commits/reopens the mixed stream exactly through the native ROM allocator, and
undoes all four records to the pre-edit expanded image. Native
