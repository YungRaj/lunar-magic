...
| Level objects | Parse standard, extended, screen-exit, custom/extension commands, and both command-zero screen-jump encodings; expose the recovered distributed six-bit command ID, command-specific parameter, orientation-neutral coordinate nibbles, screen-advance bit, packed jump target, and layout-stride-resolved screen while retaining extension bytes | Insert, delete, reorder, move, bit-preserving typed field/jump-target mutation in pristine, MWL, and interpretation-bound custom-record editors, enumerate `$01`–`$3F` through a hex-filterable active-tileset visual picker with authenticated-footprint or explicit labeled fallback cells, enumerate active-variant OSC displays by hexadecimal pair or description and construct their command-derived native extension shape, navigate a fixed-scale two-axis ROM canvas, visually insert ordinary objects at an absolute tile, drag them across all 32 native screens on either orientation, and duplicate/remove the focused Layer 1 or object-backed Layer 2 selection with Insert/Delete, stably reorder by absolute screen, regenerate minimal advance/jump transitions, preserve extensions and trailing opaque controls, copy/paste, serialize boundary-sized streams, canonically normalize/observe interpretation-bound `LMLVL1` transfer files, and prove cross-screen relocation by reciprocal Lunar Magic 3.63 MWL import/export, including low-first and high-first nonzero-component jump imports | Bank limit, terminator handling/collisions, command-zero visual insertions, invalid command IDs/coordinate nibbles/jump targets/lengths, off-canvas/minor-overflow/out-of-range-screen drops, interleaved unknown command-zero controls, typed edits that change record shape or reinterpret control objects, confusing OSC compact metadata length with native stream width, output aliases/collisions, wrong sprite-length interpretation, and rollback |
...
Native bitmap-to-Map16 file loading format-detects PNG and Windows BMP before entering the shared
preview/session path. BMP decoding is bounded to single-plane indexed 1-/4-/8-bit, default RGB555,
24-bit BGR, 32-bit BGRX, 16-/32-bit `BI_BITFIELDS`, `BI_RLE4`, or `BI_RLE8`. It decodes packed
indexed pixels through bounded BGRA palette tables, validates bitfield masks as
nonzero/disjoint/contiguous/in-range, scales each channel to eight bits, and accepts every RLE
encoded, absolute, delta, EOL, and EOB form with required word padding. Padded top-down and
bottom-up raw rows are accepted while the invalid top-down RLE combination rejects. Reserved
palette/BGRX bytes are opaque. Bad signatures, DIB extents, dimensions, planes, depth, compression,
palette shapes or indexes, bit masks, RLE extents/termination, offsets, arithmetic overflow, or
...
Acts-Like section lengths, and rejects row wrap or namespace overflow. Native import captures its
ROM revision and optional selected-tile destination before background loading; native export
snapshots the staged rectangle and publishes create-new. Tests cover cross-page rows, retargeting,
...
payload rejection, and DIB-versus-embedded dimension mismatch. Both codecs retain the importer's
bounded output allocation before any preview or staged ROM mutation is constructed.
...
| External tools/plugins | Parse bounded `LMTOOLS1` configuration and event subscriptions; install/list it through the runnable shell; capture immutable revision/level/physical-ROM emulator requests | Test shell-free command-template expansion, explicit preview and execution, automatic application-owned open/confirmed-save/real-level-change event routing, separately displayed typed launch fields, direct argument-vector process construction, working directories, successful completion, private create-new current-ROM staging, chosen or configured emulator launch, and optional level-context expansion | Duplicate IDs, truncation/oversize, unknown placeholders/events, Unicode and whitespace paths, missing context, repeated same-level selection, multi-token identifiers, atomic replacement failure, non-blocking typed event diagnostics, missing direct `{rom}` emulator argument, failed process starts, signals, nonzero exits, queue overflow, denial, Stop, and frontend drop; reap owned emulator processes and remove every private staged ROM on all terminal paths; automatic event effects require an explicit shell execution command |
...
Localization persistence now retains Lunar Magic's original distinction between automatic system
language selection and fixed built-in English. The native Language menu exposes a localized
Auto-detect action; `auto-detect`, `builtin-en`, and an explicit catalog remain distinct across
application saves. Automatic selection clears a prior fixed catalog, matches installed locales in
preferred order exactly and then by primary language, and safely remains in English when none
matches. Adding the localized action is append-only: complete 183-key catalogs and the historical
19-key chrome catalogs both upgrade with only the new English fallback and re-encode canonically.
The six catalog tests, nine preference tests, and eight frontend/localization audits cover the
bounded upgrade, persistence, matching, and fully localized menu route.
...
Mouse shortcut parity is append-only and end-to-end. `ShortcutKey::MouseLeft` through
`MouseExtra2` encode as kinds 17–21 without renumbering the original keyboard schema; the shortcut
editor round-trips stable `mouseleft`/`mouseright`/`mousemiddle`/`mouseextra1`/`mouseextra2` text.
The original user-toolbar adapter accepts both the five named `VK_*BUTTON` tokens and their exact
raw `$01/$02/$04/$05/$06` forms. `pointer_translation_preserves_all_five_native_buttons` and
...
An application-backed SSC catalog fixture also selects the same `$F0` sprite number as independent
four-, five-, six-, and seven-byte custom records across all four extra-bit tables, preserves
current placement, verifies every zero-filled extension, semantically edits packed coordinates
without changing any width or table selector, commits/reopens the mixed stream exactly through the
native ROM allocator, and undoes all four records to the pre-edit expanded image. Native
...
Semantic `LMLEDIT1` custom-time tests cover enabled `$ABC`, disabled, `$1000` overflow, and the
non-persistable zero-without-force form. The application-shell test commits both horizontal and
...
requested properties, repairs the checksum, and undoes the complete ROM byte-exactly. A direct
aggregate-controller failure test proves an invalid memory value cannot partially change assets or
history.

Semantic entrance automation uses the bounded `LMENTR1` grammar. Parser tests cover exact main and
midway fields, original Layer 2 preset `$0F`, malformed arities, invalid hex, preset `$10`, unknown
commands, and aggregate bounds. The application test runs a main edit followed by preset `$0A`,
proves the preset preserves the staged low position nibble, reopens all four physical planes from
an ExLoROM project, checks the repaired checksum, and undoes the whole ROM byte-exactly. Direct
controller batches prove a late invalid preset or unavailable midway record cannot expose an
earlier staged main edit; a terminal midway request also leaves the ROM unchanged when the
installed runtime cannot authenticate. The focused native editor suite covers the corresponding
detected-mapper entrance-controller initialization.

Native secondary-exit automation uses bounded `LMSEXED1` scripts. Parser/model tests cover ordered
set, clear-current, clear-all, a post-clear replacement, all eight semantic fields, index `$2000`,
bad hex and arity, unknown commands, byte limits, and command-count limits. The application-shell
test starts from an authentic Lunar Magic installed-ROM fixture, clears all 8,192 records, sets and
then clears `$0400`, installs `$0401`, and proves exact semantic reopen, checksum repair, one ROM
undo back to the complete fixture, and failure atomicity when a late destination `$2000` fails
native table validation.

Semantic native screen-exit automation extends `LMLEDIT1` with `object screen-exit INDEX SCREEN
DESTINATION_AND_FLAGS`. Model tests prove `$1000` becomes required-flagged `$1400`, changes a
compact record into the canonical five-byte parameter-2 form, preserves the advance-screen bit,
and rolls back an earlier edit when a late target is not a screen exit. The application-shell test
independently commits requested `$0000` and `$1000`, reopens canonical `$0400` compact and `$1400`
extended records, checks checksum repair and exact whole-ROM undo after each, and proves both a
late wrong-record failure and source screen `$20` leave the baseline ROM unchanged.

Absolute object automation adds `LMLEDIT1 object place` and `object relocate-position` coverage.
Parser tests cover the complete typed edits plus screen `$20`, coordinate `$10`, and noncanonical
boolean rejection. The application-shell test places a real ordinary object on screen `$1F` with
minor coordinate `$1C`, proves a canonical screen-jump transition and the explicit perpendicular
high bit survive ROM reopen, then independently relocates the baseline object to screen `$1E` and
minor `$1A`. Both paths repair the checksum and undo the whole ROM byte-exactly; a late index 999
...
retaining identity fields. Portable-controller tests cover both framing modes; installed and
portable canvas tests prove they use the same typed edits. The application-shell gate places an
expanded record at screen `$1F`, X `$0C`, Y `$009D`, reopens its extra bits, then independently
relocates a baseline record to screen `$1E`, X `$0A`, Y `$008F`. Both paths repair checksum and undo
the whole ROM byte-exactly; late index 999 and expanded Y `$1000` failures roll back a preceding
header edit.

Semantic sprite-field automation adds `LMLEDIT1 sprite fields INDEX Y_LOW EXTRA_BITS SCREEN X
SPRITE_NUMBER`. Model tests prove custom extensions and expanded upper-Y survive, legacy and
expanded streams reorder canonically, the resulting token index follows the record, and a
revision-width mismatch rolls the stream back atomically. Portable document and installed
aggregate controller tests exercise the typed edit rather than raw replacement; both GUI editor
paths track selection after reordering. Parser tests cover every packed bound. The application-shell
gate edits a real expanded record, reopens its screen, coordinates, number, and extra bits with a
valid checksum, then undoes the ROM byte-exactly.
...
The fifth `sprite-spawn` child has a separately bounded `LMSPAWN1` grammar. Loader
tests prove it emits only semantic range/Smart-Spawn and boundary-air intent, while
aggregate-controller tests prove current Lfix3/expanded-settings availability, preservation of the
shared high flags and GFX selector, checksum-valid reopen, history undo/redo, exact ROM undo, and
...
buffers. The no-neighborhood, method-1, and method-2 captures now match Rust exactly across both
active palette rows and the complete native `$000–$2FF` graphics workspace. A direct return capture
proves method 2's 16-color selector output is exact, while an independent `004f0269` boundary
capture proves its complete pre-allocation source-color plane is byte-identical. Focused allocator
