...
| Lunar restore points | Decode the exact `$130`-byte `LR` archive prefix and linked `$100`-byte `DIRL` records with 64-bit reciprocal links, record IDs, packed timestamps, ROM sizes, descriptions, payload bounds, raw-DEFLATE flag, and decoded byte-sum checksum; decode compact fill/raw commands with 24/32-bit destinations and one- through four-byte lengths | Reconstruct a selected record by applying the complete linked delta chain to the original ROM and resizing to every record's declared ROM size; synthetic compressed commands pass, and an authenticated Lunar Magic 3.63 two-record archive expands to 525 commands and reproduces the 2,097,664-byte target ROM exactly | Reject short/bad headers, invalid signatures, record cycles, broken previous links, mismatched last links, overlapping/out-of-bounds headers, descriptions, and payloads, failed inflation/checksum, malformed/truncated/unterminated commands, trailing decoded bytes, arithmetic overflow, unknown record IDs, and outputs above the 16-MiB ROM bound |
