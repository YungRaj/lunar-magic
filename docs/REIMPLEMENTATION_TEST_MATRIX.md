...
24-bit BGR, 32-bit BGRX, or 16-/32-bit `BI_BITFIELDS`. It decodes packed indexed pixels through
bounded BGRA palette tables, validates bitfield masks as nonzero/disjoint/contiguous/in-range, scales
each channel to eight bits, accepts padded top-down and bottom-up rows, and treats reserved
palette/BGRX bytes as opaque. Bad signatures, DIB extents, dimensions, planes, depth, compression,
palette shapes or indexes, bit masks, offsets, arithmetic overflow, or truncated pixels reject
without producing a partial image.
