...
| Lifecycle | Full-ROM restore points and associated-file restore | P | P | P | ~ | - | Partial | Exact `.lrp` header, linked `DIRL` records, record extents, stored/decoded checksums, raw-DEFLATE payloads, compact fill/raw commands, logical-ROM CRC-32, and bounded reconstruction pass; an LM 3.63 two-record archive restores the 2,097,664-byte target ROM byte-exactly from `smwOrig.smc`. All thirteen Ghidra-confirmed sidecar slots decode with incremental inheritance, per-record compression, and the pre-3.21 large-file offset rule. Native one-record full archive creation round-trips compressed/uncompressed ROM deltas and sidecars; the closed-project dialog atomically publishes the ROM and selected associated files. Add append/delta/reversion creation, automatic policy, native creation controls, and authentic nonempty-sidecar fixtures |
