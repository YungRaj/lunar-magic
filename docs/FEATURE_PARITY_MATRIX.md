...
| Lifecycle | Full-ROM restore points and associated-file restore | P | P | P | ~ | - | Partial | Exact `.lrp` header, linked `DIRL` records, record extents, stored/decoded checksums, raw-DEFLATE payloads, compact fill/raw commands, logical-ROM CRC-32, and bounded reconstruction pass; an LM 3.63 two-record archive restores the 2,097,664-byte target ROM byte-exactly from `smwOrig.smc`. All thirteen Ghidra-confirmed sidecar slots decode with incremental inheritance, per-record compression, and the pre-3.21 large-file offset rule. Native creation supports initial and appended full checkpoints, appended deltas, reciprocal link/header updates, complete timestamp snapshots, and compressed/uncompressed ROM/sidecar data. Reconstruction starts at the latest preceding full checkpoint. The closed-project dialog atomically publishes the ROM and selected associated files. Add reversion creation, automatic full-point policy, native creation controls, and authentic nonempty-sidecar fixtures |
