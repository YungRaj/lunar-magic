...
| Graphics | Extract/insert GFX and ExGFX | P | P | ~ | ~ | ~ | Partial | CLI/project operations pass. The installed graphics editor non-blockingly extracts its staged slot as create-new raw 4bpp `GFX%02X.bin`/`ExGFX%02X.bin` and boundedly inserts an exact-size raw file through the same ownership-aware staged transaction. Its standard-GFX directory action decodes every profile-declared pointer-table entry from one immutable ROM image, stages fixed `GFX%02X.bin` names with per-file progress and Escape/button cancellation, and publishes the complete set all-or-rollback. AllGFX joining, all-file insertion, installed ExGFX enumeration, and broader original prompts remain incomplete |
