...
| Graphics | Extract/insert GFX and ExGFX | P | P | ~ | ~ | ~ | Partial | CLI/project operations pass. The installed graphics editor now non-blockingly extracts its staged slot as create-new raw 4bpp `GFX%02X.bin`/`ExGFX%02X.bin` and boundedly inserts an exact-size raw file through the same ownership-aware staged transaction; all-file directory workflows and broader original prompts remain incomplete |
