...
| Graphics | Extract/insert GFX and ExGFX | P | P | ~ | ~ | ~ | Partial | CLI/project operations pass. The installed graphics editor non-blockingly extracts its staged slot as create-new raw 4bpp `GFX%02X.bin`/`ExGFX%02X.bin` and boundedly inserts an exact-size raw file through the same ownership-aware staged transaction. Its standard-GFX directory actions use fixed `GFX%02X.bin` names, bounded background I/O, per-file progress, and Escape/button cancellation. Extraction decodes every profile-declared pointer-table entry from one immutable ROM image and publishes the complete set all-or-rollback. Insertion requires the complete exact-shape set, precompresses every file, then allocates, repoints, semantically reopens, checksum-repairs, and commits the ROM as one revision or leaves it unchanged. Equivalent create-new `AllGFX.bin` extraction concatenates those raw slots in table order; bounded joined insertion derives exact split offsets from the current ROM and enters the same atomic transaction. Canonical profiles now represent contiguous or three-plane graphics pointers, validate and audit their physical components, and protect every plane during allocation. On the exact pristine SMW layout this enables separate native directory extraction and atomic insertion of the recovered nonsequential special pair (`GFX33`, then `GFX32`) under their public filenames. Installed ExGFX enumeration and broader original prompts remain incomplete |
