...
| Lifecycle | Full-ROM restore points and associated-file restore | P | P | P | ~ | - | Partial | Exact `.lrp` header, linked `DIRL` records, record extents, stored/decoded checksums, raw-DEFLATE payloads, compact fill/raw commands, logical-ROM CRC-32, and bounded reconstruction pass; an LM 3.63 two-record archive restores the 2,097,664-byte target ROM byte-exactly from `smwOrig.smc`. All thirteen Ghidra-confirmed sidecar slots decode with incremental inheritance, per-record compression, and the pre-3.21 large-file offset rule. Native creation supports initial/appended full checkpoints, deltas, successful/failed reversion markers, consecutive-reversion replacement, reciprocal link/header updates, complete timestamp snapshots, and compressed/uncompressed ROM/sidecar data. Reconstruction follows the non-linear reversion graph. Automatic policy selects continuity-break, interval, daily, or delta behavior in original priority order. The closed-project dialog atomically publishes the ROM and selected associated files; an open project can create and safely publish a one-record full archive including all thirteen sidecar slots, with a native round-trip fixture covering nonempty sidecars. Add native append/automatic/reversion creation controls and authentic Lunar Magic nonempty-sidecar fixtures |
