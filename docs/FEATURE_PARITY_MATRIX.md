...
| Level | Change bypassed GFX/ExGFX and animation options | P | P | P | ~ | - | Partial | Ghidra-proved expanded-header mapping now has a lossless typed model and controls for six FG/BG plus four SP 12-bit files in both the portable and installed native-level-assets editors. Installed edits use the checked undoable aggregate transaction and preserve unknown bits. A profile-backed resolver decodes all ten selected files at native 2/3/4bpp depths and materializes fixed VRAM slots. The live installed-assets workspace can rasterize a real tilemap-backed Layer 2 from those slots together with profile Map16 definitions and the staged palette. The four inverted animation-feature bits now have named lossless semantics, MWL checkbox/script routing, an exact checked installed-ROM sentinel/table migration model, a runtime-relative table locator with an independent feature-hook marker, and strict primary/fallback revision-profile routing with complete allocation protection. Feature-runtime installation, installed GUI/aggregate-save routing, object-stream/full interactive rendering, variants, and Wine dialog/save fixtures remain |
