...
| Level | Change bypassed GFX/ExGFX and animation options | P | ~ | ~ | ~ | - | Partial | Ghidra-proved expanded-header mapping now has a lossless typed model and advanced editor controls for six FG/BG plus four SP 12-bit files, preserving unknown bits. Complete installed-ROM transaction/render integration, animation options, variants, and Wine dialog/save fixtures remain |
