...
| Level | Export one/all level images as PNG/BMP | P | P | ~ | P | ~ | Partial | Renderer/CLI pass, and the installed native-assets editor exports the current staged level as create-new PNG or bottom-up padded 24-bit BMP using the full pre-viewport software canvas and exact native level bounds; batch export and broader original prompt/naming behavior remain |
