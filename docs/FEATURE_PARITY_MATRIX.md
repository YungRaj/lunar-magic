...
| Lifecycle | Full-ROM restore points and associated-file restore | P | ~ | ~ | ~ | - | Partial | Exact `.lrp` header, linked `DIRL` records, raw-DEFLATE payloads, checksums, compact fill/raw commands, logical-ROM CRC-32, and bounded reconstruction pass; an LM 3.63 two-record archive restores the 2,097,664-byte target ROM byte-exactly from `smwOrig.smc`. The closed-project native dialog lists the recovered fields and publishes the selected point off-thread through atomic replacement. Add archive creation and thirteen-sidecar restore |
