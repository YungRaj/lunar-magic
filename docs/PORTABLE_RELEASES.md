...
```sh
cd dist
shasum -a 256 -c lunar-magic-rust-1.0.1-x86_64-unknown-linux-gnu.tar.gz.sha256
tar -tzf lunar-magic-rust-1.0.1-x86_64-unknown-linux-gnu.tar.gz
tar -xOzf lunar-magic-rust-1.0.1-x86_64-unknown-linux-gnu.tar.gz \
  lunar-magic-rust-1.0.1-x86_64-unknown-linux-gnu/RELEASE-MANIFEST.txt
```
...
the same tamper-checking path. Non-tag runs produce `1.0.1-dev` CI artifacts retained for 14 days.
