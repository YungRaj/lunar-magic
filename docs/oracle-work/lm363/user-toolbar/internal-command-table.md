...
command by its original ID, and currently routes 178 authenticated table entries to native commands,
...
commands: ordinary 2/3/4 MiB, warned 8 MiB ExLoROM conversion, and warned SA-1 6/8 MiB. This closes
command enumeration and prevents false acceptance, but does not claim that the remaining commands
are implemented.
