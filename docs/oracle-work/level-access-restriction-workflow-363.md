...
6. when the ROM restore-point feature flag at `005e628e` is set, calls
...
The restore-point flag is not a generic application preference: cross-references show that it is
decoded and encoded by `DecodeRomFeatureOptionFlags`/`EncodeRomFeatureOptionFlags`, and is also
consulted by `PrepareAutomaticRestorePoint` and other destructive bulk-save commands.
...
`restriction/persist -> IPS offer -> optional standard IPS workflow -> completion notice -> close`
...
write leaves the project open with Retry Save and Close. The remaining restore-point hook must be
driven by the ROM feature flag rather than guessed from the independent global restore-policy
controls.
