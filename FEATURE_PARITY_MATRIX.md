...
| Map16 | Import bitmap with quantization, deduplication, previews | P | P | ~ | ~ | ~ | Partial | Native PNG and bounded background system-clipboard workflows load real level graphics/palette context, show aspect-correct synchronized original/converted pan-and-zoom previews, expose optimization/reuse/flip/deduplication/priority choices plus byte-preserving fixed-color reservations, and commit palette/GFX/Map16/runtime changes atomically; reproduce the original multi-row color-allocation priorities, alternate reduction choices, and broader Wine variants |
