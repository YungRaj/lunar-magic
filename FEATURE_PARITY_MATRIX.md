...
| Map16 | Import bitmap with quantization, deduplication, previews | P | P | ~ | ~ | ~ | Partial | Native PNG workflow loads real level graphics/palette context, shows aspect-correct synchronized original/converted pan-and-zoom previews, exposes optimization/reuse/flip/deduplication/priority choices, and commits palette/GFX/Map16/runtime changes atomically; reproduce the original color-allocation dialog, palette reservations, alternate quantizer choices, clipboard entry, and broader Wine variants |
