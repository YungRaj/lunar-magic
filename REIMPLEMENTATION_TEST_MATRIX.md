...
On macOS, the opt-in `snes9x_smoke` gate expands the supplied pristine fixture through the Rust
project API, repairs its checksum, launches only the generated temporary ROM in Snes9x, requires
the emulator process to survive initialization, and then terminates it and removes the temporary
copy. This is launch-time runtime evidence, not a substitute for screenshot- or input-driven
gameplay assertions.
