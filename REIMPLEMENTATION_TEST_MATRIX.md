...
| Level objects | Parse standard, extended, screen-exit, custom/extension commands, and both command-zero screen-jump encodings; expose the recovered distributed six-bit command ID, command-specific parameter, orientation-neutral coordinate nibbles, screen-advance bit, and packed jump target while retaining extension bytes | Insert, delete, reorder, move, bit-preserving typed field/jump-target mutation, drag ordinary objects across all 32 native screens on either orientation, stably reorder by absolute screen, regenerate minimal advance/jump transitions, preserve trailing opaque controls, copy/paste, serialize boundary-sized streams, canonically normalize/observe interpretation-bound `LMLVL1` transfer files, and prove cross-screen relocation by reciprocal Lunar Magic 3.63 MWL import/export | Bank limit, terminator handling/collisions, invalid command IDs/coordinate nibbles/jump targets/lengths, off-canvas/minor-overflow/out-of-range-screen drops, interleaved unknown command-zero controls, typed edits that change record shape or reinterpret control objects, output aliases/collisions, wrong sprite-length interpretation, and rollback |
...
| Sprites | Parse legacy/current `yyyyEESY / XXXXssss / NNNNNNNN` records, expanded upper-Y/control tokens, escaped `FF` records, and exact-bounded revision-table-sized extra bytes | Insert, move, reorder, delete, losslessly edit X/Y/screen/extra-bit/number fields, drag placements on horizontal and vertical native canvases, stably restore Lunar Magic's cross-screen legacy ordering without changing within-screen priority, compatibility conversion, checked native/project/interchange serialization, semantic reopen, direct-ROM Wine relocation/edit/export equivalence, and target-addressable observations of native record/upper-Y/control tokens | Per-level limits, unknown custom sprite records, extension preservation, short public records, legacy control/terminator collisions, expanded/control stream position sorting, out-of-range upper-Y tokens and canvas drops, shape-changing field edits, reserved `FE`/`FF` controls, malformed/oversized length tables, and bytes hidden after inner stream terminators; reject before ROM/history mutation |
