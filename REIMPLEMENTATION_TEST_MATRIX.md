...
| Level objects | Parse standard, extended, screen-exit, custom/extension commands, and both command-zero screen-jump encodings; expose the recovered distributed six-bit command ID, command-specific parameter, orientation-neutral coordinate nibbles, screen-advance bit, and packed jump target while retaining extension bytes | Insert, delete, reorder, move, bit-preserving typed field/jump-target mutation, copy/paste, serialize boundary-sized streams, and canonically normalize/observe interpretation-bound `LMLVL1` transfer files | Bank limit, terminator handling/collisions, invalid command IDs/coordinate nibbles/jump targets/lengths, typed edits that change record shape or reinterpret ordinary objects, output aliases/collisions, wrong sprite-length interpretation, and rollback |
...
| Sprites | Parse legacy/current records, expanded screen/control tokens, escaped `FF` records, and exact-bounded revision-table-sized extra bytes | Insert, move, reorder, delete, compatibility conversion, checked native/project/interchange serialization, semantic reopen, and target-addressable observations of native record/screen/control tokens | Per-level limits, unknown custom sprite records, extension preservation, short public records, legacy control/terminator collisions, out-of-range screen tokens, reserved `FE`/`FF` controls, malformed/oversized length tables, and bytes hidden after inner stream terminators; reject before ROM/history mutation |
