...
| Sprites | Parse legacy/current `yyyyEESY / XXXXssss / NNNNNNNN` records, expanded upper-Y/control tokens, escaped `FF` records, and exact-bounded revision-table-sized extra bytes | Insert, move, reorder, delete, losslessly edit X/Y/screen/extra-bit/number fields, compatibility conversion, checked native/project/interchange serialization, semantic reopen, and target-addressable observations of native record/upper-Y/control tokens | Per-level limits, unknown custom sprite records, extension preservation, short public records, legacy control/terminator collisions, out-of-range upper-Y tokens, shape-changing field edits, reserved `FE`/`FF` controls, malformed/oversized length tables, and bytes hidden after inner stream terminators; reject before ROM/history mutation |
