...
On macOS, the opt-in `snes9x_smoke` gates expand the supplied pristine fixture through the Rust
project API and independently edit/repoint installed level `$105` Layer 2. Both paths repair the
checksum, semantically reopen the generated state, launch only the temporary output in Snes9x,
require the emulator process to survive initialization, and then terminate it and remove the
