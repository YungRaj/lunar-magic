...
| Level objects | Parse standard, extended, screen-exit, custom/extension commands, and both command-zero screen-jump encodings; expose the recovered distributed six-bit command ID, command-specific parameter, orientation-neutral coordinate nibbles, screen-advance bit, and packed jump target while retaining extension bytes | Insert, delete, reorder, move, bit-preserving typed field/jump-target mutation, drag ordinary objects within their current native screen on either orientation, copy/paste, serialize boundary-sized streams, canonically normalize/observe interpretation-bound `LMLVL1` transfer files, and prove typed coordinate edits by reciprocal Lunar Magic 3.63 MWL import/export | Bank limit, terminator handling/collisions, invalid command IDs/coordinate nibbles/jump targets/lengths, off-canvas/minor-overflow/cross-screen drops, typed edits that change record shape or reinterpret ordinary objects, output aliases/collisions, wrong sprite-length interpretation, and rollback |
