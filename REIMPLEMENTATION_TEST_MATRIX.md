...
standard Layer 1 object, and independently edit/repoint installed level `$105` Layer 2 through the
Rust project API. Every path repairs the checksum, semantically reopens the generated state,
launches only the temporary output in Snes9x, requires the emulator process to survive
initialization, and then terminates it and removes the temporary copy. This is launch-time runtime
evidence, not a substitute for screenshot- or input-driven gameplay assertions.
