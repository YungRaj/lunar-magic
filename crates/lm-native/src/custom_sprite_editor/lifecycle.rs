...
impl CustomSpriteEditor {
    pub(crate) fn request_close(&mut self, application: bool) -> bool {
        if self.loader.is_running() {
            self.error = Some("wait for custom-sprite loading to finish before closing".into());
            return false;
        }
        if self.persistence.is_running() {
            self.error = Some("wait for custom-sprite persistence to finish before closing".into());
            return false;
        }
