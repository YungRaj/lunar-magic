    ...
    fn update(&mut self, context: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        self.recovery_store
            .synchronize_project(recovery_revision, || {
                ...
                if staged_editors > 1 {
                    return Err(
                        "cannot compose simultaneous staged level, level-assets, expanded settings, legacy graphics bypass, title recording, shared palette, overworld/boss messages, secondary exits, overworld level names/player starts/settings/special events/event numbers/event reveals/event tilemaps, graphics, ExAnimation, title/credits tilemap, palette, Map16, or overworld recovery yet".into(),
                    );
                }
