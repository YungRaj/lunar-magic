...
impl NativeApplication {
    pub(super) fn show_rom_editors(&mut self, context: &egui::Context) {
        ...
        if let Some(command) = command
            && self.try_dispatch(context, command)
        {
            self.rom_lunar_magic_metadata_editor.commit_succeeded();
            self.renderer.invalidate();
        }
        if quit {
            self.request_quit(context);
        ...
        let (quit, command) = self.rom_boss_sequence_editor.show(
            context,
            self.app.project_revision(),
            self.app.localization(),
        );
        if let Some(command) = command
            && self.try_dispatch(context, command)
        {
            self.rom_boss_sequence_editor.commit_succeeded();
            self.renderer.invalidate();
        }
        if quit {
            self.request_quit(context);
        }
        let (quit, command) = self.rom_overworld_message_editor.show(
            context,
            self.app.project_revision(),
            self.app.localization(),
        );
        if let Some(command) = command
            && self.try_dispatch(context, command)
        {
            self.rom_overworld_message_editor.commit_succeeded();
        ...
        let (quit, command) = self.rom_overworld_event_reveal_editor.show(
            context,
            self.app.project_revision(),
            self.app.localization(),
        );
        if let Some(command) = command
            && self.try_dispatch(context, command)
        {
            self.rom_overworld_event_reveal_editor.commit_succeeded();
            self.mark_user_toolbar_save_notification(
                lm_app::LunarMagicNotificationKind::SaveOverworld,
            );
            self.renderer.invalidate();
        ...
        let (quit, command) = self.rom_overworld_event_tilemap_editor.show(
            context,
            self.app.project_revision(),
            self.app.localization(),
        );
        if let Some(command) = command
            && self.try_dispatch(context, command)
        {
            self.rom_overworld_event_tilemap_editor.commit_succeeded();
            self.mark_user_toolbar_save_notification(
                lm_app::LunarMagicNotificationKind::SaveOverworld,
            );
            self.renderer.invalidate();
        ...
        let (quit, command) = self.rom_overworld_special_event_editor.show(
            context,
            self.app.project_revision(),
            self.app.localization(),
        );
        if let Some(command) = command
            && self.try_dispatch(context, command)
        {
            self.rom_overworld_special_event_editor.commit_succeeded();
            self.mark_user_toolbar_save_notification(
                lm_app::LunarMagicNotificationKind::SaveOverworld,
            );
            self.renderer.invalidate();
        ...
        let (quit, command) = self.rom_exanimation_editor.show(
            context,
            self.app.project_revision(),
            self.app.localization(),
        );
        if let Some(command) = command
            && self.try_dispatch(context, command)
        {
            self.rom_exanimation_editor.commit_succeeded();
            self.renderer.invalidate();
        }
        if quit {
            self.request_quit(context);
