...
pub(crate) fn execute_command(
    ...
    match command {
        crate::command_types::Command::SmwTitleTilemapExport { rom, output } => {
            export(rom, output)?;
        }
        crate::command_types::Command::SmwTitleTilemapImport {
            input_rom,
            tilemap,
            output_rom,
...
fn export(rom: &Path, output: &Path) -> Result<(), Box<dyn std::error::Error>> {
    if rom == output {
        return Err("title tilemap output must differ from ROM input".into());
    }
...
fn import(
    ...
    if input_rom == output_rom || tilemap == output_rom {
        return Err("title tilemap output must differ from every input".into());
    }
    ...
    if project.load_title_tilemap_detected(locator)?.tilemap != tilemap {
        return Err("title tilemap semantic reopen mismatch".into());
    }
    write_new(output_rom, project.save_snapshot())?;
    println!("imported-title-tilemap-words: 1856");
    Ok(())
}
...
fn open_smw_us_v1(path: &Path) -> Result<Project, Box<dyn std::error::Error>> {
    ...
    {
        return Err("title tilemaps require SMW US revision 0 LoROM".into());
    }
    Ok(project)
}
