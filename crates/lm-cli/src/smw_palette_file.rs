...
use lm_oracle::observe_smw_palette;
use std::path::Path;
...
pub fn execute(
    ...
    let file = SmwPaletteFile::decode(&read_bounded(input, SmwPaletteFile::MAX_FILE_LEN)?)?;
    println!("backend: {:?}", file.backend());
    println!("colors: {}", file.palette()?.colors.len());
    let encoded = normalized.map(|_| file.encode());
    let observed = observation
        .map(|_| observe_smw_palette(&file).map(|value| value.to_text()))
        .transpose()?;
...
fn validate_paths(
    ...
    if [normalized, observation]
        ...
        .any(|path| path == input)
    {
        return Err("native palette outputs must differ from input");
    }
    if normalized.is_some() && normalized == observation {
        return Err("normalized native palette and observation outputs must differ");
    }
    Ok(())
}
