...
pub fn parse(
    ...
    let (normalized, observation) = match text {
        [command, _, _, _] if command == "overworld-file" => (None, None),
        [command, _, _, _, _] if command == "overworld-file" => (Some(4), None),
        [command, _, _, _, _, _] if command == "overworld-file" => (Some(4), Some(5)),
        _ => return Ok(None),
    };
    Ok(Some(Command::OverworldFile {
        input: PathBuf::from(&args[1]),
        size_modes: PathBuf::from(&args[2]),
        maximum_animation_records: usize::try_from(parse_number(&text[3])?)
            .map_err(|_| ArgsError("maximum animation record count does not fit usize".into()))?,
