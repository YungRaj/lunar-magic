...
fn parse_special_command(
    ...
    if let Some(command) = crate::arg_native_overworld_appearance::parse(args, text) {
        return Ok(Some(command));
    }
...
fn parse_profile_command(
    ...
    Ok(match text {
        ...
        [command, _, _, _] if command == "smw-map16-complete-export-template" => {
            Some(Command::SmwMap16CompleteExport {
                rom: PathBuf::from(&args[1]),
                template: Some(PathBuf::from(&args[2])),
                output: PathBuf::from(&args[3]),
            })
        }
        [command, _, _, _] if command == "smw-map16-complete-import" => {
            Some(Command::SmwMap16CompleteImport {
                input_rom: PathBuf::from(&args[1]),
