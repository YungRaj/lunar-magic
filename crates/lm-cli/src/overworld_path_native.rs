...
use lm_profile::{SMW_US_V1_CHECKSUM_FIELD, smw_us_v1_overworld_path_link_layout};
use lm_profile::{
    smw_us_v1_overworld_path_installation_plan, smw_us_v1_overworld_path_patch_locator,
...
const MAX_LINK_FILE_BYTES: usize = 12 + OverworldPathLinkTable::MAX_LINKS * 12;

pub(crate) fn execute_command(
    command: &crate::command_types::Command,
) -> Result<bool, Box<dyn std::error::Error>> {
    match command {
        crate::command_types::Command::SmwOverworldPathExport { rom, output } => {
            export(rom, output)?;
        }
        crate::command_types::Command::SmwOverworldPathImport {
            input_rom,
            links,
            output_rom,
...
pub(crate) fn export(rom: &Path, output: &Path) -> Result<(), Box<dyn std::error::Error>> {
    if rom == output {
        return Err("native overworld path output must differ from ROM input".into());
    }
...
pub(crate) fn import(
    ...
    if input_rom == output_rom || links == output_rom {
        return Err("native overworld path output must differ from every input".into());
    }
    let table =
        OverworldPathLinkTable::decode_native_file(&read_bounded(links, MAX_LINK_FILE_BYTES)?)?;
    ...
    match loaded.storage {
        OverworldPathLinkStorage::Fixed if table.links.len() == 14 => {
            project.save_overworld_path_links(
                &table,
    ...
    let reopened = project
        ...
        .table;
    if reopened != table {
        return Err("native overworld path semantic reopen mismatch".into());
    }
    write_new(output_rom, project.save_snapshot())?;
    println!(
        "imported-native-overworld-path-links: {}",
        table.links.len()
    );
    Ok(())
}
...
fn open_smw_us_v1(path: &Path) -> Result<Project, Box<dyn std::error::Error>> {
    ...
    {
        return Err("native overworld paths require SMW US revision 0 LoROM".into());
    }
    Ok(project)
}
