...
impl AppState {
    pub(crate) fn replace_credits_tilemap(
        &mut self,
        ...
        if !save_native_credits_tilemap_to_project(project, tilemap)? {
            return Ok(Vec::new());
        }
        self.advance_project_revision()?;
        let description = "Replace native SMW credits tilemap".to_owned();
...
pub fn save_native_credits_tilemap_to_project(
    project: &mut Project,
    tilemap: &CreditsTilemap,
    ...
    {
        return Err(AppError::CreditsTilemapIdentityMismatch);
    }
    ...
    if project.load_credits_tilemap_detected(&locator)?.tilemap != *tilemap {
        return Err(AppError::CreditsTilemapReopenMismatch);
    }
    Ok(true)
}
...
mod tests {
    ...
    #[test]
    fn capacity_safe_edit_is_undoable_to_the_exact_pristine_rom() {
