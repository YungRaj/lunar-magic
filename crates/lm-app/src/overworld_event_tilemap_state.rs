...
impl AppState {
    pub(crate) fn replace_native_event_tilemaps(
        ...
        if !save_native_overworld_event_tilemaps_to_project(project, buffers)? {
            return Ok(Vec::new());
        }
...
/// Saves all native event-tilemap planes through the same detected pristine/installed path as the
/// ordinary application command. Returns `false` when the current decoded buffers are exact.
pub fn save_native_overworld_event_tilemaps_to_project(
    project: &mut Project,
    buffers: &EventTilemapBuffers,
